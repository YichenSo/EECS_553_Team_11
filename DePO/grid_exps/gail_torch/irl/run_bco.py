...
from gail_torch.policy import PPOPolicy, DiscretePPOPolicy, Discriminator, BCO
...
def do_train(config):
    ...
    tot_metrics = {"jsd": [], "expert_density": [], "agent_density": []}
    ...
    for update_cnt in range(config["exp_params"]["num_updates"]):
        ...
        if update_cnt % 10 == 0:
            ...
            memory, _ , _ = eval_sampler.collect_samples(1024)
    ...
    with open("bco_{}.pkl".format(config["env_specs"]["repeat_ratio"]), "wb") as f:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("BCO for single agent environments")
    parser.add_argument(
        "--exp_config",
        "-e",
