...
from gail_torch.policy import PPOPolicy, DiscretePPOPolicy, Discriminator, DPO
...
def do_train(config):
    ...
    for update_cnt in range(config["exp_params"]["num_updates"]):
        ...
        metrics = policy.update_sp()
        metrics = policy.update_inv(memory)
    ...
    with open("dpo_sup_{}.pkl".format(config["env_specs"]["repeat_ratio"]), "wb") as f:
