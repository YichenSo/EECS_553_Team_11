...
gaifo_metrics.pop("agent_density")
bco_metrics.pop("expert_density")
dpo_metrics.pop("expert_density")
gaifo_metrics.pop("expert_density")
...
for idx, key in enumerate(gaifo_metrics.keys()):
    plt.plot(
        np.arange(len(gaifo_metrics[key])),
        gaifo_metrics[key],
        linestyle=":",
        color=colors[idx],
    )
...
patches = [
    ...
    Line2D([0], [0], color="black", lw=2, label="GAIfO", ls=":")
]
