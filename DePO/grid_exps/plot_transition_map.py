...
import sys
sys.path.append("gail_torch")

# append gridworld path
...
if __name__ == "__main__":
    ...
    # for method_name in ["dpo_sup"]:

        model_path = "./gail_torch/assets/learned_models/{}/model_final.pkl".format(
            method_name
        )
        expert_path = "/home/theo-yifan/EECS553_Project/DePO/grid_exps/gail_torch/assets/expert_traj/expert.pkl"
        ...
        # for each subarray in expert_density, print the shape of the subarray
        temp_subarray = np.zeros((6, 6))
        # save each subarray in expert_density as a separate image
        for i, subarray in enumerate(expert_density):
            # sum the subarray along the first axis
            subarray = np.sum(subarray, axis=0)
            subarray = subarray.reshape(6, 6)
            temp_subarray += subarray

        expert_heatmap = data_to_img(temp_subarray)
        expert_density = temp_subarray
